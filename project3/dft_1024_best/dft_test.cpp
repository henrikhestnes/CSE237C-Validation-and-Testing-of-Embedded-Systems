...
int main()
{
	...
	{
		In_R[i].data = static_cast<uint>(i);
		In_I[i].data = static_cast<uint>(0);
	}
	...
	for(int i=0; i<SIZE; i++)
	{
		...
		rmse_R.add_value((float)(reinterpret_cast<float&>(Out_R[i].data) - gold_R));
		rmse_I.add_value((float)(reinterpret_cast<float&>(Out_I[i].data) - gold_I));
	}
	fclose(fp);
