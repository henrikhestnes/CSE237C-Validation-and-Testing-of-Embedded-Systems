...
extern "C" void cordiccart2pol_hw_stub_wrapper(float, float, volatile void *, volatile void *);

extern "C" void apatb_cordiccart2pol_hw(float __xlx_apatb_param_x, float __xlx_apatb_param_y, volatile void * __xlx_apatb_param_r, volatile void * __xlx_apatb_param_theta) {
    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > r_pc_buffer(1);
          int i = 0;
          ...
          if (i > 0) {((char*)__xlx_apatb_param_r)[0*4+0] = r_pc_buffer[0].range(7, 0).to_int64();
((char*)__xlx_apatb_param_r)[0*4+1] = r_pc_buffer[0].range(15, 8).to_int64();
((char*)__xlx_apatb_param_r)[0*4+2] = r_pc_buffer[0].range(23, 16).to_int64();
((char*)__xlx_apatb_param_r)[0*4+3] = r_pc_buffer[0].range(31, 24).to_int64();
}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
  {
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > theta_pc_buffer(1);
          int i = 0;
          ...
          if (i > 0) {((char*)__xlx_apatb_param_theta)[0*4+0] = theta_pc_buffer[0].range(7, 0).to_int64();
((char*)__xlx_apatb_param_theta)[0*4+1] = theta_pc_buffer[0].range(15, 8).to_int64();
((char*)__xlx_apatb_param_theta)[0*4+2] = theta_pc_buffer[0].range(23, 16).to_int64();
((char*)__xlx_apatb_param_theta)[0*4+3] = theta_pc_buffer[0].range(31, 24).to_int64();
}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
// print theta Transactions
{
  ...
  {
    sc_bv<32> __xlx_tmp_lv = *((int*)__xlx_apatb_param_theta);
