...
constexpr int m_size = 256;  // Must be a multiple of 8.
...
//    queue q(default_selector{}, dpc_common::exception_handler);
    ...
    q.submit([&](auto &h) {
      ...
      // Execute kernel.
      h.template parallel_for<b_init>(range(N, P), [=](auto index) {
    ...
    q.submit([&](auto &h) {
      ...
      h.template parallel_for<c_calc>(range(M, P), [=](auto index) {
        ...
        for (int i = 0; i < width_a; i+=8) {
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i) * PrefetchingLSU::load(b_ptr + i * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+1) * PrefetchingLSU::load(b_ptr + (i+1) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+2) * PrefetchingLSU::load(b_ptr + (i+2) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+3) * PrefetchingLSU::load(b_ptr + (i+3) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+4) * PrefetchingLSU::load(b_ptr + (i+4) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+5) * PrefetchingLSU::load(b_ptr + (i+5) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+6) * PrefetchingLSU::load(b_ptr + (i+6) * width_b + col );
          sum += PrefetchingLSU::load(a_ptr + row * width_a + i+7) * PrefetchingLSU::load(b_ptr + (i+7) * width_b + col );
        }
