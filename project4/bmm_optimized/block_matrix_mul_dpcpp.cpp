...
int main() {
 ...
 /*     auto A = aa.get_access<access::mode::read>(h);
   ...
   */     
      accessor A(aa, h, read_only);
      accessor B(bb, h, read_only);
      ...
      h.template parallel_for<c_calc>(nd_range<2>(num_groups, num_items), [=](nd_item<2> item) {
          ...
          for (int a = a_start, b = b_start; a <= a_end; a += BL, b += (BL * P)){
              ...
              A_local[local_y][local_x] = PrefetchingLSU::load(a_ptr + a + N * local_y + local_x);
              B_local[local_x][local_y] = PrefetchingLSU::load(b_ptr + b + P * local_y + local_x);
